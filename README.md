...
### Сайт - Решение
Создаю Instance Group с тремя ВМ, Target Group, Backend Group, HTTP router и Application load balancer с помощью Терраформа. Так же для сайта создаю внешний IP адрес и прописываю в DNS A запись сайта, который поддерживает с помощью Yandex Certificate Manager HTTPS. Адрес сайта - https://jo-os.ru

При помощи Ансибла устанавливаю на ВМ nginx и копирую папку с сайтом.
...
### Мониторинг - Решение
Создаю с помощью Терраформа 2 ВМ - сервер и веб, так же создаю базу данных через Yandex Managed Service for 
PostgreSQL.

При помощи Ансибла устанавливаю сервер с подключением к бд, далее устанавливаю веб сервер и после установки веб сервера через api создаю группу Netology и дашбоард с метриками Netology. На остальные ВМ устанавливаю Zabbix Agent и с помощью api при установке добавляю ВМ в Zabbix server.
...
### Логи - Решение
Терраформом создаю ВМ для Elasticsearch и Kibana.

При помощи Ансибла устанавливаю на Elasticsearch - докер контейнеры Elasticsearch и Logstash, на Kibana - докер контейнер Kibana. На ВМ устанавливаю докер контейнеры Filebeat и в зависимости от ВМ собираю логи nginx или syslog.
...
Терраформом создаю один VPC с подсетями для публичных и внутренних серверов (2 подсети для разных регионов), далее создаю ВМ для Bastion и файл настройки ssh config для использования bastion при подключении по ssh к другим ВМ.
Создаю и присваиваю по именам необходимые группы безопасности. Открываю трафик внутри сетей с ВМ.
Для внешнего трафика открыты только bastion, web сайт, zabbix web и kibana.
...
При помощи Терраформа - yandex_compute_snapshot_schedule - орагизуем резервное копирование - делаем снимки ВМ раз в день.
